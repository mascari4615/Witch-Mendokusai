...
namespace WitchMendokusai.DomainSDK.Idle
{
    ...
    /// <summary>
    /// 인형 한 종류의 <b>변하지 않는 몫</b> — 뽑기 전에 이미 정해져 있는 것.
    ///
    /// ★ 종류(Kind)와 <b>가진 것</b>(<see cref="IdleDollOwned"/>)을 나눈다.
    ///   저장에는 「몇 번을 몇 ★ 로 가졌나」만 들어가고, 성능표는 코드·SO 에 있다 —
    ///   그래야 수치를 고쳐도 남의 저장이 안 깨진다.
    /// </summary>
    public readonly struct IdleDollKind
    {
        public IdleDollKind(int id, string name, IdleDollAxis axis, IdleDollGrade grade, int sides)
            : this(id, name, axis, grade, sides, IdleCards.SkillForAxis(axis))
        {
        }

        public IdleDollKind(int id, string name, IdleDollAxis axis, IdleDollGrade grade, int sides, IdleCardKind skill)
        {
            Id = id;
            Name = name;
            Axis = axis;
        ...
        /// <summary>이 인형이 미는 축.</summary>
        public IdleDollAxis Axis { get; }
    ...
    /// <summary>
    /// 인형 등급. 뽑을 때의 확률과 몫의 크기를 함께 정한다.
    ///
    /// ★ 네 단계 (업계 관측 3~9, 인디 기본값 4). 더 늘리면 <b>차이가 안 보이고</b>,
    ///   줄이면 뽑기의 오르내림이 없어진다.
    /// </summary>
    public enum IdleDollGrade
    {
        Common = 0,
        Rare = 1,
        Epic = 2,
    ...
    /// </summary>
    [Serializable]
    public struct IdleDollOwned
    {
        /// <summary><see cref="IdleDollKind.Id"/>.</summary>
        public int Id;
