...
{
    // IdleDolls.cs 의 Multipliers 조각. 같은 클래스의 partial. 상태(필드)는 원본 파일을 본다. 보유, 도감, 편성 배수와 사거리.
    public static partial class IdleDolls
    {
        /// <summary>
        /// 한 축의 <b>보유 배수</b> — 같은 갈래는 더하고, 축끼리는 부르는 쪽이 따로 곱한다.
        /// </summary>
        public static double OwnedMultiplierOf(IdleState state, IdleTuning tuning, IdleDollAxis axis)
        {
            double sum = 0d;
        ...
        public static int DiscoveryScoreOf(IdleState state)
        {
            ...
            for (int index = 0; index < state.Dolls.Count; index++)
            {
                score += 1 + state.Dolls[index].Stars;
            }
        ...
        /// <summary>
        /// 편성한 얼굴이 주는 몫. <b>편성한 인형만</b> 이 배수를 준다(보유 효과와 별개).
        ///
        /// ★ 보유는 가지고만 있어도, 편성은 내보내야. 둘을 갈라야
        ///   <b>누구를 내보낼까</b>가 결정이 된다 — 안 그러면 전원 참전이 늘 정답이다.
        /// ★ 메인 칸과 보조 칸의 몫이 다르다. 보조는 전장에 안 서서 맞지도 않으니
        ///   같은 몫이면 늘 보조가 정답이 된다. 보조 몫은 <see cref="IdleTuning.DollSupportShareByGrade"/>.
        /// </summary>
        public static double PartyMultiplierOf(IdleState state, IdleTuning tuning, IdleDollAxis axis)
        {
            double sum = 0d;
            ...
            for (int slot = 0; slot < state.Party.Length; slot++)
            {
                ...
                double share = IsMainSlot(slot)
                    ? tuning.DollPartyShareByGrade
                    : tuning.DollSupportShareByGrade;
        ...
        /// </summary>
        public static double AxisMultiplierOf(IdleState state, IdleTuning tuning, IdleDollAxis axis)
        {
        ...
        public static double RangeOf(IdleState state, IdleTuning tuning, int seat)
        {
            ...
            double[] table = tuning.DollRangeByAxis;
