...
//   왕복 200ms 회선에서 「끊고 · 붙고 · 첫 그림」이 얼마나 걸리는지가 그 값이다.
{
	...
	// [문턱-사유] (b) 사람이 느끼는 선 — 3초는 「끊겼다」로 읽기 시작하는 자리다(넉넉한 바닥).
	//   진짜 제품 주장은 아래 「넘는 그 순간」 줄이 한다.
	...
	if (crossedAt === 0) {
		...
		// [문턱-사유] (b) 사람이 느끼는 선 — 실측 125~357ms 의 4배. 넘는 순간의 정적은
		//   회선(200ms)+붙기+첫 그림이라 기계 몫이 작다(그리기가 아니라 오가는 시간이다).
