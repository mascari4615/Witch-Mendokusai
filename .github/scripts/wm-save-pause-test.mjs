#!/usr/bin/env node
// wm-save-pause-test.mjs — <b>기억을 적는 동안 세계가 멎지 않나</b> (TASK-WM-353).
//
...
//   ③ 세계가 한 번도 오래 멎지 않았나
//
// 실행: node .github/scripts/wm-save-pause-test.mjs
...
function cannotRun(message) {
	console.error(`[저장멈춤] CANNOT-RUN: ${message}`);
	process.exit(2);
}
...
if (bad === 0) {
	console.log('[저장멈춤] ✅ 큰 세계를 적는 동안에도 사람들의 세계는 흐른다');
	process.exit(0);
}
