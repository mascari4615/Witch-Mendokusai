	...
	public sealed class SlowWindowTests
	{
		...
		public async Task 안_읽는_창에게는_사람_수를_줄여_준다()
		{
			...
			try
			{
				...
				// ⚠ 줄이 <b>막히기까지</b>는 이 기계가 얼마나 빠른가에 달렸다 — 판을 빨리 찍어 낼수록
				//   안 읽는 창의 버퍼가 빨리 찬다. 2코어 러너에서는 20초 안에 안 막혔다(CI 실측 2026-08-12).
				//   그건 세계의 고장이 아니라 <b>여기서는 못 재는 것</b>이다. 빨강으로 우기면
				//   태생적 빨강이 되고, 그러면 사람이 이 시험을 끈다.
				if (narrowed == 0)
				{
					Assert.Ignore("이 기계에서는 안 읽는 창의 줄이 20초 안에 안 막혔다 — 여기선 못 판단한다"
						+ " (셈 자체는 InterestCrowdTests 가 지킨다)");
				}
