	...
	public sealed class WorldHost
	{
		...
		private static readonly float INTEREST_CELL_SIZE =
			float.TryParse(System.Environment.GetEnvironmentVariable("WM_INTEREST_CELL"), out float askedCell) && askedCell > 0
				? askedCell
				// ★ 24m 은 <b>재서</b> 고른 값이다 (TASK-WM-396 · 400). 봇 200명을 펼쳐 놓고 재니:
				//   칸  8m — 지은 그림 52855 · 쓰레기 1070MB · 나간 바이트 187.9MB
				//   칸 16m — 27707 · 711MB · 193.5MB   (옛 값)
				//   <b>칸 24m — 17417 · 563MB · 195.3MB</b>
				//   칸 48m — 14601 · 577MB · 196.6MB · 칸 96m — 13855 · 553MB · 195.5MB
				//
				// ⚠ <b>48m 을 써 보고 되돌렸다</b> (TASK-WM-400): 지은 그림은 27% 줄지만
				//   한 벌이 덮는 폭이 <b>반경 + 칸</b>이라 32+48 = <b>80m</b> 가 된다 —
				//   시험 셋이 곧바로 빨갰다(「70m 밖 사람이 실려 온다」). 세계가 약속한 것은 32m 다.
				//   칸을 키우는 값은 <b>약속을 넓히는 값</b>으로 치른다 — 그건 싼 게 아니다.
				//   (몰린 칸이 스스로 쪼개진다고 해서(WM-398) 넓히기가 공짜가 되는 건 아니었다.)
				: 24f;
		...
		private async Task HandOverAsync(int dollId, Connection socket, string zoneName, string zoneAddress, Vector3 landing)
		{
			...
			// ★ 여기서 <b>기억까지 지우지는 않는다</b> (TASK-WM-385).
			//   전에는 지웠다 — 「들고 간 것을 이 세계도 기억하면 돌아왔을 때 두 벌」(WM-259) 이라서다.
			//   그런데 통행증은 30초를 산다. 회선이 나쁘거나 창이 느려 그 안에 저쪽에 못 붙으면
			//   그 사람은 <b>손님</b>으로 도착하고(도장이 죽었다), 여기 기억은 이미 없다 ⇒
			//   가방·자리·몸이 <b>통째로</b> 사라진다(실측: 20개 → 0). 복제보다 나쁘다 — 그 사람이 잃는다.
			//   지금은 <b>도착이 확인될 때</b> 지운다(`landed`, WM-377·378·381) — 짐은 도착이 확인된 뒤에 옮긴다.
			...
			bool fieldFromScratch = false, int fieldVersionNow = 0, int windowSawFieldVersion = 0)
		{
			...
			// ⚠ <b>알려진 흠</b> (TASK-WM-397): 칸에 상한보다 많이 모이면 한복판 기준으로 자르는데,
			//   그때 <b>칸 구석에 선 사람</b>은 0.9m 옆 친구의 움직임을 <b>한 번도</b> 못 받는다(실측 8초에 0번).
			//   「사람마다 제 옆 몇 명을 얹기」로 고쳐 봤더니 초록이 됐지만(움직임 40번),
			//   광장(400명 한자리)에서 나가는 바이트가 <b>83MB → 530MB</b> 로 터졌다 — 그건 반쪽 고침이다.
			//   근본은 <b>몰린 칸을 쪼개는 것</b>(칸을 반으로 줄여 다시 나눠 쓰기)이다. 다음 판에 한다.
