...
MUTATIONS = [
    ...
    ("되돌려 쓰기가 조건 종류를 무시",
     ...
     'if (false)\n\t\t\t{'),
    ("줄이 꽉 차도 그냥 받음",
     'Assets/_WitchMendokusai/Domain/Narrative/Scripts/DialoguePlayQueue.cs',
    ...
    ("같은 대화를 두 번 줄 세움",
     ...
     'if (false)\n\t\t\t\t{\n\t\t\t\t\treturn false;\n\t\t\t\t}'),
    ("읽는 시간 아래 한계를 안 지킴",
     'Assets/_WitchMendokusai/Domain/Narrative/Scripts/DialogueReadingTime.cs',
     'if (seconds < minimumSeconds)\n\t\t\t{\n\t\t\t\tseconds = minimumSeconds;\n\t\t\t}',
     'if (false)\n\t\t\t{\n\t\t\t\tseconds = minimumSeconds;\n\t\t\t}'),
    ("아무도 안 부르는 장면을 안 알림",
     'Assets/_WitchMendokusai/Domain/Narrative/Scripts/Script/DialogueScriptParser.cs',
