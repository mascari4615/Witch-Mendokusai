...
namespace WitchMendokusai.EditorTools
{
	...
	public static class TowerDefenseOutpostVerify
	{
		...
		public static void Run()
		{
			...
			Vector3 core = match.CoreCombatant != null ? match.CoreCombatant.Position : match.StageRoot.position.ToSim();
			int outpostsBuilt = 0;
			...
			Vector3 coreLocal = match.StageRoot.InverseTransformPoint(core.ToUnity()).ToSim();
			...
			foreach (Vector3 local in byDistance)
			{
				if (tried >= 20)
					break;
				tried++;
				match.TryPlaceHarvester(match.StageRoot.TransformPoint(local.ToUnity()).ToSim());
			}
