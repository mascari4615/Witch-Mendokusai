...
{
	/// <summary>
	/// TASK-WM-173 — <b>시연용 더미</b>. 「샘 → 중계석 → 공방」 세 거점을 깔고, 한 시간마다 마력을 흘려
	...
	public class LeylineDemoPulse : MonoBehaviour
	{
		...
		[Tooltip("한 시간마다 흘려보내는 마력 양.")]
		[SerializeField] private float sendPerHour = 20f;

		[Tooltip("한 시간마다 콘솔에 남길지. 꺼 두면 돌아도 눈에 안 보인다.")]
		[SerializeField] private bool logEachHour = true;
		...
		public void Configure(LeylineDirector target, string demoSource, string demoRelay, string demoSink,
			float demoSourceToRelay, float demoRelayToSink, float demoSendPerHour)
		{
			director = target;
			sourceId = demoSource;
			relayId = demoRelay;
		...
		{
			float arrived = director.Send(sourceId, sinkId, sendPerHour);

			if (logEachHour == true)
			{
				float lost = sendPerHour - arrived;
				Debug.Log($"[마력의 강] {sourceId} → {sinkId} : 보낸 {sendPerHour:0.#} → 도착 {arrived:0.#}"
					+ $" (오는 길에 {lost:0.#} 샘)");
			}
		}
	}
