...
namespace WitchMendokusai.Idle
{
	...
	///
	/// ★ 수치 노출 룰의 방치형 적용, Unity 비의존 DomainSDK 대신 SO 소유
	///   SO 값을 <see cref="ToTuning"/> 으로 코어에 전달
	///   코드 수정 없는 수치 조정
	///
	/// ★ 비용, 성장 배수: 난이도 핵심 변수
	...
	public sealed class TuningSO : ScriptableObject
	{
		[Header("대상, 단계마다 증가")]
		[Tooltip("1단계 대상의 체력.")]
		...
		[Tooltip("떠 있는 시간. 짧을수록 포착의 중요도가 커진다.")]
		...
		[Tooltip("폭주. 판 전체(공격 속도, 기지 생산)에 적용되는 배수.")]
		...
		[Tooltip("점수 하나가 곱하는 배수. 영향이 가장 큰 성장 변수.")]
		...
		[Tooltip("초당 차는 코스트. 자동 전투와 카드 개입 계열 실측 기준 0.4 (1칸 = 약 2.4초).")]
		...
		[Tooltip("구역마다 적 피해에 곱해지는 배수. 체력 곡선보다 완만하게 설정.")]
