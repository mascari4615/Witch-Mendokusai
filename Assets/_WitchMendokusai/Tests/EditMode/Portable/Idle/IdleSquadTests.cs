...
namespace WitchMendokusai.Tests
{
	...
	public sealed class IdleSquadTests
	{
		...
		///
		/// 인형 하나 추가 착석, 앞자리(시작 인형)를 일부러 눕힌 뒤 게이지만큼 진행
		/// </summary>
		[Test]
		...
		public void TheSnapshotCarriesTheSeats()
		{
			...
			Assert.AreEqual(IdleDolls.StarterId, snapshot.Seats[0].DollId, "0번 자리가 시작 인형이 아니다");
		...
		/// <summary>파티 자리에 인형을 앉히고 체력을 세운다.</summary>
		private static void Seat(IdleState state, IdleTuning tuning, int partySlot, int dollId)
		{
			state.Dolls.Add(new IdleDollOwned(dollId));
			state.Party[partySlot] = dollId;
			state.EnsureSeatRoom(tuning);
		}
	}
}
