...
namespace WitchMendokusai.Tests
{
	...
	/// </summary>
	public sealed class IdleDollTests
	{
		...
		public void Pulling_CostsRebirthStones()
		{
			...
			Assert.IsFalse(IdleGacha.TryPull(state, tuning, out IdleDollPull _), "빈손인데 뽑혔다");

			// 자원만 있어도 안 되고, 환생석만 있어도 안 된다 — 둘 다 낸다.
			state.Resource = IdleGacha.CostOf(state, tuning);
			Assert.IsFalse(IdleGacha.TryPull(state, tuning, out IdleDollPull _), "환생석 없이 뽑혔다");

			state.Stones = IdleGacha.StoneCostOf(tuning);
			Assert.IsTrue(IdleGacha.TryPull(state, tuning, out IdleDollPull got));
			Assert.AreEqual(0d, state.Resource, 1e-6d, "자원 값을 안 치렀다");
			Assert.AreEqual(0L, state.Stones, "환생석 값을 안 치렀다");
			Assert.AreEqual(1, state.Dolls.Count, "뽑았는데 도감에 안 들어왔다");
		...
		public void Duplicates_BecomeStars()
		{
			...
			int before = state.Dolls[0].Stars;
			...
			Assert.Greater(state.Dolls[0].Stars, before, "중복을 먹였는데 ★ 이 그대로다");
		}

		/// <summary>★ 첫 인형은 <b>빈 자리에 스스로</b> 앉는다 — 뽑았는데 아무 일도 안 나면 안 된다.</summary>
		[Test]
		public void FirstDolls_TakeEmptyPartySlots()
		{
			IdleTuning tuning = new IdleTuning();
			IdleState state = new IdleState();
			...
			Assert.Greater(seated, 0, "뽑은 인형이 아무 자리에도 안 앉았다");
		}
		...
		public void OwningAlone_MakesYouStronger()
		{
			...
			owning.Dolls.Add(new IdleDollOwned(0));
		...
		public void SendingOut_BeatsJustOwning()
		{
			...
			fighting.Dolls.Add(new IdleDollOwned(0));
			fighting.Party[0] = 0;
		...
		/// <summary>
		/// ★ 축이 갈린다 — 공격 인형은 공격만, 기지 인형은 기지만 민다.
		///
		/// 이게 무너지면 「무엇을 뽑았나」가 아무 뜻이 없어지고 그냥 숫자 하나가 된다.
		/// </summary>
		[Test]
		public void EachDoll_PushesItsOwnAxis()
		{
			IdleTuning tuning = new IdleTuning();
			...
			IdleState baseDoll = new IdleState();
			baseDoll.EnsureProducerRoom(tuning.ProducerCount);
			baseDoll.Dolls.Add(new IdleDollOwned(1)); // 네모 = 기지 축

			Assert.Greater(IdleBase.OutputPerSecond(baseDoll, tuning), IdleBase.OutputPerSecond(plain, tuning),
				"기지 인형인데 기지가 그대로다");
			Assert.AreEqual(IdleModel.AttackSpeedOf(plain, tuning), IdleModel.AttackSpeedOf(baseDoll, tuning), 1e-9d,
				"기지 인형이 공격속도까지 올렸다 — 축이 안 갈렸다");
		}
		...
		public void SameAxisAdds_DiscoveryMultiplies()
		{
			...
			state.Dolls.Add(new IdleDollOwned(0));
			double one = IdleDolls.OwnedMultiplierOf(state, tuning, IdleDollAxis.Damage);

			state.Dolls.Add(new IdleDollOwned(4)); // 쐐기 = 같은 공격 축, 레어
			double two = IdleDolls.OwnedMultiplierOf(state, tuning, IdleDollAxis.Damage);

			// 합이므로 <b>배수의 초과분</b>이 정확히 더해진다.
			double firstShare = IdleDolls.OwnedShareOf(state.Dolls[0], tuning);
			double secondShare = IdleDolls.OwnedShareOf(state.Dolls[1], tuning);
		...
		public void Discovery_RisesInSteps()
		{
			...
			double before = IdleDolls.DiscoveryMultiplierOf(state, tuning);
			...
			Assert.Greater(IdleDolls.DiscoveryMultiplierOf(state, tuning), before, "문턱을 넘었는데 안 올랐다");
		}
		...
		public void Pity_EventuallyGivesLegend()
		{
			...
			{
				if (IdleGacha.TryPull(state, tuning, out IdleDollPull got) == false)
				{
					break;
				}

				if (got.Grade == IdleDollGrade.Legend)
				{
					sawLegend = true;
					break;
				}
		...
		public void OneFace_CannotTakeTwoSlots()
		{
			...
			Assert.AreNotEqual(state.Party[0], state.Party[1], "같은 인형이 두 자리에 앉았다");
		}

		/// <summary>도감과 파티가 저장을 건넌다 — 안 그러면 껐다 켤 때마다 처음부터 모은다.</summary>
		[Test]
		public void DollsSurviveSaveLoad()
		{
			IdleTuning tuning = new IdleTuning();
			IdleState state = new IdleState();
			...
			Assert.AreEqual(state.Dolls.Count, restored.Dolls.Count);
			Assert.AreEqual(state.Party[0], restored.Party[0]);
			Assert.AreEqual(state.PullsSincePity, restored.PullsSincePity);
		}

		/// <summary>옛 저장에는 인형 없음. 예외 없이 시작 인형 하나를 받아 들어옴 (C10)</summary>
		[Test]
		public void OldSaves_LoadWithNoDolls()
		{
			IdleState fromOld = new IdleState();
			fromOld.Load(new IdleSaveData { Resource = 5d });

			Assert.IsNotNull(fromOld.Dolls);
			Assert.AreEqual(1, fromOld.Dolls.Count, "옛 저장에 시작 인형을 안 줬다");
			Assert.AreEqual(IdleDolls.StarterId, fromOld.Party[0]);
			Assert.AreEqual(IdleDolls.PARTY_SLOTS, fromOld.Party.Length);
		}
		...
		/// <summary>시험용 — 원하는 인형이 나올 때까지 뽑는다(주사위를 건드리지 않는다).</summary>
		...
		public void PublishedOdds_MatchTheRealRoll()
		{
			...
			for (int one = 0; one < rolls; one++)
			{
				...
				if (IdleGacha.TryPull(state, tuning, out IdleDollPull got) && got.Grade == IdleDollGrade.Legend)
				{
					legend++;
				}
			}
		...
		/// <summary>
		/// ★ 자리를 <b>비우면</b> 그 인형이 사라진다 — 다른 빈 자리로 복제되지 않는다 (회귀).
		///
		/// 빈 자리는 -1 로 적힌다. 맞바꿈 규칙(같은 인형이 두 자리를 못 먹는다)을 빼는 요청에도
		/// 그대로 태우면 -1 끼리 「같은 인형」으로 잡혀 [5,-1,-1] 이 [-1,5,5] 가 됐다.
		/// 그러면 한 명이 세 자리 중 두 자리를 먹으면서 배수도 두 번 세어진다 —
		/// 셋을 고르는 결정이 통째로 무너진다.
		/// </summary>
		[Test]
		public void EmptyingASeat_DoesNotCloneTheDoll()
		{
			IdleTuning tuning = new IdleTuning();
			IdleState state = new IdleState();
		...
		/// <summary>★ 이미 선 인형을 다른 자리에 앉히면 <b>맞바꾼다</b> — 같은 얼굴이 두 자리를 못 먹는다.</summary>
		[Test]
		public void SeatingADollAlreadyStanding_SwapsInstead()
		{
			IdleTuning tuning = new IdleTuning();
			IdleState state = new IdleState();
			state.EnsureProducerRoom(tuning.ProducerCount);
			state.Dolls.Add(new IdleDollOwned(5));
			state.Dolls.Add(new IdleDollOwned(9));
			state.Party[0] = 5;
			state.Party[1] = 9;
		...
		public void TheDiscoveryBonus_LandsExactlyOnce()
		{
			...
			for (int id = 0; id < tuning.DiscoveryStepScore; id++)
			{
				read.Dolls.Add(new IdleDollOwned(id));
			}

			double discovery = IdleDolls.DiscoveryMultiplierOf(read, tuning);
			...
			double owned = IdleDolls.OwnedMultiplierOf(read, tuning, IdleDollAxis.Speed)
				* IdleDolls.PartyMultiplierOf(read, tuning, IdleDollAxis.Speed);
			...
			double ownedBase = IdleDolls.OwnedMultiplierOf(read, tuning, IdleDollAxis.Base)
				* IdleDolls.PartyMultiplierOf(read, tuning, IdleDollAxis.Base);
		...
		/// <summary>
		/// ★ 저장에 <b>모르는 인형 번호</b>가 있어도 판이 선다 — 그 얼굴만 버린다.
		///
		...
		/// </summary>
		[Test]
		public void AnUnknownDollInTheSave_IsDropped()
		{
			IdleSaveData saved = new IdleState().Save();
			saved.Dolls = new IdleDollOwned[] { new IdleDollOwned(0), new IdleDollOwned(9999) };
			saved.Party = new int[] { 9999, 0, -1 };

			IdleState state = new IdleState();
			state.Load(saved);

			Assert.AreEqual(1, state.Dolls.Count, "모르는 얼굴을 그대로 받았다");
			Assert.AreEqual(0, state.Dolls[0].Id);
			...
			Assert.IsNotNull(session.Capture().Dolls);
		}

		/// <summary>★ 가지지도 않은 얼굴이 자리에 적혀 있으면 그 자리는 빈다.</summary>
		[Test]
		public void APartySeatWithoutTheDoll_ComesBackEmpty()
		{
			IdleSaveData saved = new IdleState().Save();
			saved.Dolls = new IdleDollOwned[0];
			...
			Assert.AreEqual(IdleDolls.StarterId, state.Party[0], "빈 자리를 시작 인형이 안 채웠다");
			Assert.Less(state.IndexOfDoll(3), 0, "안 가진 인형이 도감에 생겼다");
			for (int slot = 0; slot < state.Party.Length; slot++)
			{
				Assert.AreNotEqual(3, state.Party[slot], "안 가진 인형이 서 있다");
			}
		}
	}
}
