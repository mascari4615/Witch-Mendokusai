	...
	public sealed class IdleStageWallProbe
	{
		...
		private static void RaiseCheapestStats(IdleState state, IdleTuning tuning, IdleUpgradeKind[] kinds)
		{
			for (int guard = 0; guard < 200; guard++)
			{
				...
				{
					if (IdleModel.TryGetCost(state, tuning, IdleDolls.StarterId, kinds[index], 1, out double cost) && cost < bestCost)
					{
						bestCost = cost;
						best = kinds[index];
					}
				...
				if (IdleModel.TryRaise(state, tuning, IdleDolls.StarterId, best, 1) == false)
				{
					return;
				}
			}
		...
		private static void MergeAndWear(IdleState state, IdleTuning tuning)
		{
			...
			for (int bag = 0; bag < state.Bag.Count; bag++)
			{
				...
				IdleItem worn = state.Worn[IdleGear.WornAt(IdleDolls.StarterId, (int)item.Slot)];
				if (worn.Tier < item.Tier && IdleGear.TryEquip(state, IdleDolls.StarterId, bag))
				{
					bag = -1;
				}
			}
