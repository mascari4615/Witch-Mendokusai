...
namespace WitchMendokusai
{
	...
	public static class TowerDefenseResearchGraph
	{
			...
			for (int branch = 0; branch < branchCount; branch++)
			{
				...
				for (int ring = 1; ring <= ringCount; ring++)
				{
					...
					if (forking == false)
					{
						...
						int id = nextId++;
						into.Add(MakeNode(id, theme, ring, ringCount, baseAngle, radius, previous));
						previous = id;
						continue;
					}
					...
					int left = nextId++;
					int right = nextId++;
					into.Add(MakeNode(left, theme, ring, ringCount, baseAngle - FORK_SPREAD, radius, previous));
					into.Add(MakeNode(right, theme, ring, ringCount, baseAngle + FORK_SPREAD, radius, previous));

					// 다음 고리는 *둘 중 아무거나* 하나면 열린다 — 두 선을 그어 그 뜻을 낸다.
					previous = left;
					int merge = nextId++;
					Node mergeNode = MakeNode(merge, theme, ring + 1, ringCount, baseAngle, RING_STEP * (ring + 1), left);
			...
			float angleDegrees, float radius, int previous)
		{
			...
			float amount = major ? 0.35f : 0.08f;
			return new Node
			{
				Id = id,
				Effect = theme,
